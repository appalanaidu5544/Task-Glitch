...
export default function ChartsDashboard({ tasks }: Props) {
  ...
  const validROI = tasks.filter(t => typeof t.roi === 'number' && Number.isFinite(t.roi));

  const roiBuckets = [
    { label: '<200', count: validROI.filter(t => t.roi! < 200).length },
    { label: '200-500', count: validROI.filter(t => t.roi! >= 200 && t.roi! <= 500).length },
    { label: '>500', count: validROI.filter(t => t.roi! > 500).length },
    { label: 'N/A', count: tasks.length - validROI.length },
  ];
